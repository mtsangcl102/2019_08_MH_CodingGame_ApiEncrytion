...
namespace BestHTTP
{
    ...
    public class HTTPResponse : IDisposable
    {
        ...
        protected byte[] DecodeStream(BufferPoolMemoryStream streamToDecode)
        {
            ...
            if (encoding == null)
           {
                Debug.Log("decode " + "encoding null");
                return streamToDecode.ToArray();
            }
            else
            {
                Debug.Log("decode " + encoding[ 0 ] );
            ...
            using (var ms = new BufferPoolMemoryStream((int)streamToDecode.Length))
            {
                ...
                {
                    try{
                        if( (byteCount = decoderStream.Read(buf, 0, buf.Length)) > 0 ){
                        }
                        else{
                            break;
                        }
                    ...
                    Debug.Log( "ms == null " + (ms == null ) +" "+ (decoderStream== null ) +" "+ (buf == null));
