...
describe('encryption', () => {
    ...
    it('encryption basic test', () => {
        ...
        const encrypted = encryptionService.encrypt(message, Buffer.from(nonce.toString()));
        console.log('encrypted:', encrypted, nonce);
    ...
    it('encryption performance test', () => {
        ...
        console.time('case2');
        for (let i = 0; i < loop; i++) {
            let nonce = 256 * 256 * 256 + i;
            const encrypted = encryptionService.encrypt(message, nonce);
            const decrypted = encryptionService.decrypt(encrypted, nonce);
